...
class UsageRecord(Base):
    ...
    request_id: Mapped[Optional[str]] = mapped_column(
        ...
        comment="Links to X-Request-ID header for full request tracing",
    )
    scores: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True,
        comment="Per-layer verdict scores and latencies from the scan pipeline",
    )
    violations: Mapped[Optional[list]] = mapped_column(
        JSON, nullable=True,
        comment="List of violation strings from the scan pipeline",
    )
