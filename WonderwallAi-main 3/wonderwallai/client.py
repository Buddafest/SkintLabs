...
class Wonderwall:
    ...
    async def scan_inbound(self, message: str) -> Verdict:
        ...
        try:
            _sr_start = time.perf_counter()
            is_on_topic, sim_score, closest = await self._semantic_router.is_on_topic(
                message
            )
            scores["semantic"] = sim_score
            scores["semantic_latency_ms"] = round((time.perf_counter() - _sr_start) * 1000, 2)
        ...
        try:
            _ss_start = time.perf_counter()
            is_safe, sentinel_result = await self._sentinel_scan.classify(message)
            scores["sentinel_latency_ms"] = round((time.perf_counter() - _ss_start) * 1000, 2)
    ...
    async def scan_outbound(self, text: str, canary_token: str = "") -> Verdict:
        ...
        """
        try:
            _ef_start = time.perf_counter()
            is_safe, cleaned_text, violations = self._egress_filter.scan(
                text, canary_token
            )
            _ef_latency = round((time.perf_counter() - _ef_start) * 1000, 2)
        except Exception as e:
        ...
        _egress_scores = {"egress_latency_ms": _ef_latency}
        ...
        return Verdict(allowed=True, action="allow", message=cleaned_text, scores=_egress_scores)
