...
class SemanticRouter:
    ...
    async def is_on_topic(self, query: str) -> Tuple[bool, float, str]:
        ...
        # Intent logging — log ALL results (not just blocks) for full auditability
        try:
            from server.observability import log_decision
            # Top 3 similarities for debugging borderline cases
            sorted_indices = np.argsort(similarities)[::-1][:3]
            top_3 = [
                {"topic": self.topics[int(i)][:40], "sim": round(float(similarities[int(i)]), 4)}
                for i in sorted_indices
            ]
            log_decision(
                "semantic_routing",
                input_summary=query[:100],
                chosen="allowed" if is_allowed else "blocked",
                confidence=max_sim,
                reason=f"sim={max_sim:.3f} vs threshold={self.threshold}, closest='{closest_topic[:50]}'",
                metadata={"top_3": top_3},
            )
        except ImportError:
            pass  # SDK used standalone
